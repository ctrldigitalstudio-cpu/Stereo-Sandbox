...
export class World {
  constructor({ seed, faceLayers, edits, onMesh, onUnload }) {
    this.seed = seed;
    ...
    this.worker.onerror = (e) => console.error('World worker error:', e.message || e);
    const editList = [];
    for (const [k, m] of this.edits) editList.push([k, [...m]]);
    this.worker.postMessage({ type: 'init', seed, faceLayers, edits: editList });
  }

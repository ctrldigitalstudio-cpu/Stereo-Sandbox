...
// 0..1, works in RGBA8 too). Metering: 16x9 samples of a 1/32-res bloom level (before the bloom
...
void main() {
  ...
  for (int y = 0; y < NY; y++) {
    for (int x = 0; x < NX; x++) {
      ...
      float l = luminance(textureLod(uMeter, uv, 0.0).rgb);
      float sky = 0.0;
      for (int k = 0; k < 4; k++) {
      ...
      wg[i] = 1.0 - sky;              // ground (non-sky) share
      wt[i] = wPos * mix(1.0, 0.2, sky);
      sum += lg[i] * wt[i];
      wsum += wt[i];
    }
  }
  ...
  float ceilingHi = mix(0.45, 2.8, day);

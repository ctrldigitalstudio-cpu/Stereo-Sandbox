...
- Shared GLSL helpers: `ign/ignFrame, linearDepth, positionFromDepth, shadowDistort, shadowCoord,
  ...
  `footprintScale()` (render px → output px: `renderScale` with TAA, else 1) and
  `pixelArtUVOut(uv, clamp)` (`pixelArtUV` with the texel footprint in output pixels). Passes the TAA
  resolve reconstructs at the output resolution (terrain, particles) scale their `textureGrad`
  derivatives by `footprintScale()` and use `pixelArtUVOut`: a TAAU mip bias of log2(renderScale). The held
  item (drawn at the output size) and the shadow pass keep `pixelArtUV`; the sun/moon edge AA is one
  output pixel wide, stars stay about one render pixel (a smaller point would be missed by most samples).
- Frame block fields for temporal anti-aliasing (see "Temporal anti-aliasing" below). With TAA on,
  ...
  - `uTAAInfo` (vec4): x = `footprintScale` (render width / output width with TAA, 1 without), y = the
    previous frame's time (the `uCamPos.w` clock; = this frame's when the history is not valid), z = 1 when
    the scene pass writes motion vectors (TAA with float targets), w = 0. `FRAME_FLOATS` = 148.
...
over ±0.5 px around the true edge; sprite planes get a one-pixel alpha coverage ramp; sprite side walls
are widened across their silhouette edge (the one not shared with the plane facing the eye) to ≥ 2 px and
box-filtered back to their true width (a sub-pixel wall adds only the light it covers); alpha-blended
(colour `SRC_ALPHA, ONE_MINUS_SRC_ALPHA`, alpha `ONE, ONE_MINUS_SRC_ALPHA`). The TAA sharpen pass leaves it
alone (held-item depth mask).
`drawSelection` sizes the outline in pixels of the image the player sees (`view.outputHeight`: the TAA
output size, else the render size), converted to render pixels, at least one render pixel.
With the scene motion attachment (below), `drawParticles` writes (0, 0, 1, 0) to location 1 (reactive).
...
2. Shadow pass (if enabled) into a `shadowRes²` DEPTH_COMPONENT24 texture: `terrain.drawShadow` (never
   jittered).
3. Scene FBO (RGBA16F colour + DEPTH_COMPONENT24 texture, render size, jittered with TAA; with TAA and
   float targets also a motion attachment, see "Temporal anti-aliasing"):
   `terrain.drawOpaque`, `overlays.drawParticles` (both write the motion attachment),
...
4. Blit scene colour + depth into copies (units 10, 11) → `terrain.drawWater` into the scene FBO (writes
   zero motion) → `overlays.drawSelection` (leaves the motion attachment alone).
...
6. `post.composite` → composite target (render size, RGBA16F; alpha = 1 − self-emission from the terrain
   ...
   composite without TAA), with its own depth (a texture with TAA, read by the sharpen pass), cleared.
...
10. Final pass to the canvas by `settings.aa`: `taa` → RCAS (AMD FSR1 robust contrast-adaptive sharpening,
    ...
    on noise) in sRGB space + dither, same size; the strength is multiplied per pixel by the resolve's
    sharpen multiplier (aux alpha, capped at 1) and is 0 on the held item; `fxaa` → FXAA 3.11 upscaling
    from render size; `off` → straight copy (or the tone map writes the canvas directly when no scaling
    is needed).
...
- Targets (output size): history colour ×2 (HDR format, linear filter), aux ×2 (same format: rg = dilated
  linear depth as 16-bit fixed point of sqrt(depth / 2048), b = accumulated weight / 32, a = sharpen
  multiplier / 2 for the final pass), output colour (HDR, alpha = 1 − self-emission) + a depth texture for
  the held item. One MRT pass writes history, aux and output. Recreated (history reset) when the canvas
  size changes; a renderScale change keeps them.
- Scene motion attachment (renderer, render size, HDR format, TAA with float targets only; `uTAAInfo.z`):
  COLOR_ATTACHMENT1 of the scene FBO, cleared to 0. xy = NDC motion beyond the static-world reprojection:
  the terrain vertex shader writes, for waving plants/leaves, `ndc(prevVP · (pos + prevWave − wave +
  camDelta)) − ndc(prevVP · (pos + camDelta))` with the wave of `uTAAInfo.y`'s time; everything else 0.
  z = reactive (particles: no motion vectors of their own). Draw buffers: terrain, particles and water
  write both; the sky and the selection outline only attachment 0.
- Resolve, per output pixel (uv at the pixel centre, unjittered):
  ...
  2. Reprojection: the front-most depth of the 3×3 (dilation); its camera-relative position from
     `uInvViewProj` at that texel, + `uCamDelta`, projected with `uPrevViewProj`, plus that texel's motion
     vector. Sky (depth 1): by direction (w = 0), blended toward the reprojection at a finite distance
     where finite content covers it: the middle of the cloud layer (weight = 4 × its coverage there;
     not from inside the layer) or, below the horizon, the far-terrain plane of `voidColor`. Motion =
     previous uv − the texel's unjittered uv.
  ...
  4. History: 5-tap Catmull-Rom at the history uv. Variance clipping in YCoCg of the exposed, reversibly
     compressed colour `c / (1 + max(c))` against mean ± 1.25 σ of the 3×3 (± 0.75 σ around reactive
     pixels, 3×3 max of the reactive flag); history weight reduced when it was far outside (÷ distance/2),
     × mag² where the surface came closer (mag = current / previous view depth: magnified history),
     capped while moving (to 30% of the maximum at ≥ 5 px/frame) and to 1 around reactive pixels.
  5. Blend: weights `wc / (1 + luma)` and `W / (1 + luma)` (W = accumulated history weight, capped at
     8 native → 12 at renderScale 0.5, 4 for RGBA8 history): ~0.1 per frame at native resolution, a fresh
     history converges as 1/n. `sharp` = saturate(W / 2) × (1 − saturate(clip distance − 1)) × (1 −
     reactive) mixes wide → narrow reconstruction and `wc` 1 → confidence: clipped (changing) history is
     refreshed evenly from the smooth reconstruction instead of only where a sample fell close (no
     checkerboard when upscaling). No history → the wide reconstruction alone (the first frame after a
     reset shows only the current frame), stored weight = confidence at native, 0.5 when upscaling.
     RGBA8 history (no float targets) is dithered ±0.5 LSB.
  6. Sharpen multiplier (aux alpha × 2) = mix(0.6, 1 + 0.6 · saturate(speed / 2 px), sharp): less RCAS on
     fresh current-frame pixels, more on history resampled by camera motion.

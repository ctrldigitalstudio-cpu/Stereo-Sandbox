...
// Wait until at least n more frames were rendered after a change (the view keeps drawing).
const rendered = async (n = 2) => {
  const f0 = await page.evaluate(() => window.__game.renderer.frameIndex);
  await page.waitForFunction(([f0, n]) => window.__game.renderer.frameIndex >= f0 + n, [f0, n], { polling: 100, timeout: 600000 });
};
...
async function setVia(key, value, { draw = true } = {}) {
  const { item } = tabOf[key];
  // A heavy preset can take tens of seconds per frame in software GL, and Playwright's clicks wait
  // for animation frames: hold drawing while interacting, then check that it keeps rendering.
  ...
  if (draw) await page.evaluate(() => window.__game.setRender(true));
}
...
for (const p of ['low', 'medium', 'high', 'ultra', 'low']) {
  ...
  await setVia('preset', p, { draw: !heavy });
  if (heavy) {

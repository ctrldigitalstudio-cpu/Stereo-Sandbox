...
import { startServer, launch, newGamePage, waitReady, frames, reporter, opt, sleep, outDir, root, state, LIGHT_SETTINGS, SETTINGS_KEY } from './lib.mjs';
...
if (want('focus')) {
  const { page, ctx, errors } = await newGamePage(browser, { width: 480, height: 270, storage: light });
  await page.goto(`${server.base}/index.html`, { waitUntil: 'load' });
  await waitReady(page, 2);
  await page.waitForFunction(() => !document.querySelector('.btn-play').disabled, null, { polling: 250 });
  await page.click('.btn-play');
  ...
  await page.evaluate(() => {
    ...
    g.setRender(false);
    g.teleport(s.x + 0.5, s.y + 0.1, s.z + 0.5, 0.6, 0);
  ...
  check(moving > 3 && drift < 1e-3, `blur while W held stops the player (speed before ${moving.toFixed(2)}, drift after ${drift.toFixed(4)})`);
  await page.keyboard.up('KeyW');
  ...
  check(Math.hypot(c[0] - b[0], c[2] - b[2]) > 0.5, 'W works again after the blur');

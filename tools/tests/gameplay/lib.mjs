...
export function startServer(pages = {}) {
  const server = http.createServer((req, res) => {
    ...
    if (pages[url] != null) {
      // A page is HTML text, or { body, headers } (e.g. a Content-Security-Policy to test under).
      const page = typeof pages[url] === 'function' ? pages[url](req) : pages[url];
      const body = typeof page === 'string' ? page : page.body;
      res.writeHead(200, { ...headers, 'Content-Type': 'text/html; charset=utf-8', ...(page.headers || {}) });
      res.end(body);
      return;
    }
  ...
  if (storage) {
    await ctx.addInitScript((storage) => {
      try {
        ...
        // Only into empty keys: a second tab of the same context must not reset the first one's save.
        for (const k in storage) if (localStorage.getItem(k) == null) localStorage.setItem(k, typeof storage[k] === 'string' ? storage[k] : JSON.stringify(storage[k]));
...
// After Play / Resume: clicks are ignored until pointer capture has been granted or refused and
// the double-click guard has passed.
// Pass a Page (with W, H) to also park the mouse at the centre: headless Chromium turns every
// locked mouse move into a jump and a jump back, so later clicks there shouldn't move it again.
export const captureSettled = async (target, center = null) => {
  await target.waitForFunction(() => !window.__game.player.input._lockPending, null, { polling: 50, timeout: 20000 });
  if (center) { await target.mouse.move(center[0], center[1]); await frames(target, 2); }
  await sleep(450);
};

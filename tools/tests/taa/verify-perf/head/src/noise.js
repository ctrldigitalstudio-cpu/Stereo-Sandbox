// Seeded randomness: PRNG, integer hashes and simplex noise (2D + 3D).
...
export function hash2(x, z, seed) {
  return hash3(x, 0x5bd1e995, z, seed);
}
...
export class Simplex {
  ...
  fbm2(x, y, octaves) {
    ...
    for (let o = 0; o < octaves; o++) {
      sum += this.noise2(x * f + o * 31.7, y * f - o * 17.3) * amp;
      norm += amp; amp *= 0.5; f *= 2.03;
    }
    return sum / norm;
  }

  // Ridged fractal in [0, 1] (sharp crests)
  ridged2(x, y, octaves) {
    let sum = 0, amp = 1, norm = 0, f = 1, weight = 1;
    for (let o = 0; o < octaves; o++) {
      let v = 1 - Math.abs(this.noise2(x * f + o * 13.1, y * f + o * 7.7));
      v *= v;
      v *= weight;
      weight = Math.min(1, Math.max(0, v * 1.6));
      sum += v * amp; norm += amp; amp *= 0.5; f *= 2.1;
    }
    return sum / norm;
  }
}

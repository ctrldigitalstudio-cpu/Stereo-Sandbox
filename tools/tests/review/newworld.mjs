...
await page.evaluate(() => {
  localStorage.setItem('blockvale.save.v1', JSON.stringify({ seed: 777, time: 0.3, player: { x: 1000.5, y: 90, z: 2000.5, yaw: 0, pitch: 0, flying: true }, hotbar: null, selected: 0, edits: JSON.stringify([['5,5', [[1234, 1]]]]) }));
...
const after = await page.evaluate(() => ({ seed: window.__game.world.seed, pos: window.__game.player.pos, edits: window.__game.world.exportEdits(), save: localStorage.getItem('blockvale.save.v1') && JSON.parse(localStorage.getItem('blockvale.save.v1')).seed }));

...
section('init / ready / want / mesh messages');
{
  ...
  check(near.length === 0, `edited chunks are evicted too; their edits live in svc.edits (${near.map((c) => `${c.cx},${c.cz}`).join(' ')})`);
  check(svc.edits.has(chunkKey(0, 0)), 'edits outlive the evicted chunk data');

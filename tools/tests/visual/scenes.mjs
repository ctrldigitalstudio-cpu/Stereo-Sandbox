...
// The title-screen camera of main.js at its first frame (time of day 0.4, just left of the sun).
export function titleCamera(gen, sp) {
  let top = sp.y;
  for (let dz = -24; dz <= 24; dz += 3) for (let dx = -24; dx <= 24; dx += 3) top = Math.max(top, gen.heightAt(Math.floor(sp.x + dx), Math.floor(sp.z + dz)) + 1);
  const a = 0.4 * Math.PI * 2, tilt = (25 * Math.PI) / 180;
  const sun = [Math.cos(a), Math.sin(a) * Math.cos(tilt), Math.sin(a) * Math.sin(tilt)];
  const sunYaw = Math.atan2(-sun[0], -sun[2]);
  return { pos: [sp.x, Math.min(128 + 24, top + 14) - 1.62, sp.z + 5], yaw: sunYaw - 0.75, pitch: -0.15 };
}

export function sceneList(sp, shore, title) {
  return [
    ...(title ? [{ name: 'title', time: 0.4, pos: title.pos, yaw: title.yaw, pitch: title.pitch, far: true }] : []),
    ...
    { name: 'torches-close', time: 0.8, pos: [sp.x + 0.5, sp.y + 1, sp.z + 1], yaw: 0.35, pitch: -0.1, edits: torchWall, editBase: [sp.x, sp.y, sp.z + 4], selected: 6 },
    ...
    { name: 'underwater-floor', time: 0.32, pos: [shore.x + 6, 48, shore.z + 4], yaw: 1.9, pitch: -0.45 },
